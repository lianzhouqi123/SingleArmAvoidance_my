...
env = gym.make('Rm63Env-s1-2', render_mode=my_render_mode)
...
num_episodes = 2000  # 总训练循环数
buffer_size = 2 ** 14  # 样本缓存数目
...
torch.save(agent.actor.state_dict(), 'actor_result_1_2_1.pth')
torch.save(agent.critic_1.state_dict(), 'critic_1_result_1_2_1.pth')
torch.save(agent.critic_2.state_dict(), 'critic_2_result_1_2_1.pth')

with open('return_list_1_2_1.csv', 'w', newline='') as f:
    writer = csv.writer(f)
...
plt.xlabel('Episodes')
plt.ylabel('Returns')
plt.title('TD3 tran 1-2 on {}'.format(env))
plt.show()
