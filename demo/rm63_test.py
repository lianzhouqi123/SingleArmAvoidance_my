...
import manipulator_mujoco.rl_program.NAF as NAF
...
qd = torch.ones(len(timestep))/57.3*0.5  # 期望转角

# for idx in np.arange(6):
for idx in [1]:
    angle = torch.zeros([6, len(timestep)])
    angle[idx, :] = qd

    # qt = 0
    current_q = torch.zeros([1, len(timestep)]).reshape([len(timestep)])
    current_v = torch.zeros([1, len(timestep)]).reshape([len(timestep)])
    current_a = torch.zeros([1, len(timestep)]).reshape([len(timestep)])
