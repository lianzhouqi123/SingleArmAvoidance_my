...
env = gym.make('Rm63Env-s5-2', render_mode=my_render_mode)
...
save_file = "5_2_1tran2_3_act2"
if not os.path.exists(save_file):
    os.mkdir(save_file)

return_list = rl.train2(env, agent, num_episodes, replay_buffer, minimal_size, batch_size, num_iteration, save_file)

torch.save(agent.actor.state_dict(), 'actor_result_5_2_1tran2_3_act2.pth')
torch.save(agent.critic_1.state_dict(), 'critic_1_result_5_2_1tran2_3_act2.pth')
torch.save(agent.critic_2.state_dict(), 'critic_2_result_5_2_1tran2_3_act2.pth')

with open('return_list_5_2_1tran2_3_act2.csv', 'w', newline='') as f:
    writer = csv.writer(f)
...
plt.xlabel('Episodes')
plt.ylabel('Returns')
plt.title('TD3 tran 7 on {}'.format(env))
plt.show()
