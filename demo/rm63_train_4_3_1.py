...
batch_size = 128  # 取样样本数
num_iteration = 2  # 一回合训练次数
...
torch.save(agent.actor.state_dict(), 'actor_result_4_3_2.pth')
torch.save(agent.critic_1.state_dict(), 'critic_1_result_4_3_2.pth')
torch.save(agent.critic_2.state_dict(), 'critic_2_result_4_3_2.pth')

with open('return_list_4_3_2.csv', 'w', newline='') as f:
    writer = csv.writer(f)
...
plt.xlabel('Episodes')
plt.ylabel('Returns')
plt.title('TD3 tran 4-3-2 on {}'.format(env))
plt.show()
