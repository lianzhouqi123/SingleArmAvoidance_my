...
torch.save(agent.actor.state_dict(), 'actor_result_4_3_1.pth')
torch.save(agent.critic_1.state_dict(), 'critic_1_result_4_3_1.pth')
torch.save(agent.critic_2.state_dict(), 'critic_2_result_4_3_1.pth')

with open('return_list_4_3_1.csv', 'w', newline='') as f:
    writer = csv.writer(f)
...
plt.xlabel('Episodes')
plt.ylabel('Returns')
plt.title('TD3 tran 4-3 on {}'.format(env))
plt.show()
