...
class Arm():
    def __init__(self, xml_path, eef_site_name, attachment_site_name, actuator_names, joint_names = None, name: str = None):
        self._mjcf_root = mjcf.from_path(xml_path)
        self.model_physics = mujoco.Physics.from_xml_path(xml_path)
        ...
        self._actuators = self._mjcf_root.find('actuator', actuator_names)
        # actuators = env.physics.named.data.actuator
        # actuators = self.model_physics.named.data.actuator
