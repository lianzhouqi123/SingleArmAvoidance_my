...
"""
env_situation1: 随机目标，固定初始,碰撞不停,无障碍
"""


class Rm63Env_s2_1(gym.Env):
    metadata = {
    ...
    def _get_reward(self):
        ...
        # 与障碍物距离
        # reward
        rg, rc, wo_far, wo_near, wt, d_danger = 20000, -250, 80, 100, -100, 0.2
        # R_T 终点
        ...
        if d_target < self.reach_level:  # 到终点
            reward = rg
            done = True
            terminated = True
        elif contact != 0:  # 碰撞
    ...
    def target_sample(self, pos_range: list, mode: str, ori_pos: list = [0, 0, 0]):
        ...
        if mode == "range_sample_mode":
            ...
            # while distance_t_o <= self.obstacle_size + self.reach_level:
            #     self.target_pose = np.array([self.target_original_pos[0] + pos_range[0] * np.random.random(),
            #                                  self.target_original_pos[1] + pos_range[1] * np.random.random(),
            #                                  self.target_original_pos[2] + pos_range[2] * np.random.random(),
            #                                  1, 0, 0, 0])
