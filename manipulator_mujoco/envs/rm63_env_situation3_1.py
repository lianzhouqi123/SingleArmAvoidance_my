...
"""
env_situation1: 固定目标，随机初始,碰撞不停,固定障碍
"""


class Rm63Env_s3_1(gym.Env):
    metadata = {
    ...
    def reset(self, seed=None, options=None) -> tuple:
        ...
        with self._physics.reset_context():
            ...
            arm_pos_init = [
                0.0 + m.pi * 0.04 * (np.random.rand() - 0.5),
                m.pi * 0.2 + m.pi * 0.04 * (np.random.rand() - 0.5),
                -m.pi * 0.4 + m.pi * 0.04 * (np.random.rand() - 0.5),
                0.0 + m.pi * 0.04 * (np.random.rand() - 0.5),
                -m.pi / 2 + m.pi * 0.04 * (np.random.rand() - 0.5),
                0.0 + m.pi * 0.04 * (np.random.rand() - 0.5)
            ]
