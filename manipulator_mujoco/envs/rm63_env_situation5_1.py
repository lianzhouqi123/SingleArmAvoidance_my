...
"""
env_situation1: 随机目标，随机初始,碰撞不停,随机障碍
"""


class Rm63Env_s5_1(gym.Env):
    metadata = {
    ...
    def reset(self, seed=None, options=None) -> tuple:
        ...
        # obstacle
        obstacle_pos_range = np.array([0.4, 0.4, 0.4])  # 固定障碍
    ...
    def target_sample(self, pos_range: list, mode: str, ori_pos: list = [0, 0, 0]):
        ...
        if mode == "range_sample_mode":
            self.target_pose = np.array([self.target_original_pos[0] + pos_range[0] * np.random.random(),
                                         ...
                                         1, 0, 0, 0])
            distance_t_o = np.linalg.norm(self.obstacle_step_position - self.target_pose[:3])
