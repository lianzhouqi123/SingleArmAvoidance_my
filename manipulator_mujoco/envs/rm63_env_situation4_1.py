...
"""
env_situation1: 随机目标，随机初始,碰撞不停,固定障碍
"""


class Rm63Env_s4_1(gym.Env):
    metadata = {
    ...
    def __init__(
        ...
        # obstacle
        self.obstacle_ori_pos = np.array([-0.7, -0.2, 0.1])
        ...
        self.obstacle(obstacle_shape="sphere", ori_pos=self.obstacle_ori_pos, quat=self.obstacle_quat,
                      size=str(self.obstacle_size), rgba=[1, 0, 0, 0], show_range=True)
    ...
    def _get_reward(self, dis_t_old, dis_o_old):
        """
        奖励
        """
        done = False  # 是否结束循环
        ...
        return reward, done, terminated, dis_t, dis_o
    ...
    def reset(self, seed=None, options=None) -> tuple:
        ...
        # obstacle
        obstacle_pos_range = np.array([0, 0, 0])  # 固定障碍
        self.obstacle_pos = np.array([-0.0, 0.0, 0.5])
        self.obstacle_control(mode="fix", ori_pos=self.obstacle_ori_pos)

        # init the position of the target
        self.target_sample(mode="fixed_sample_mode", pos_range=[0, 0, 0])
    ...
    def step(self, action) -> tuple:
        ...
        reward, done, terminated, self.dis_t_old, self.dis_o_old = self._get_reward(self.dis_t_old, self.dis_o_old)
        info = self._get_info()
    ...
    def obstacle_control(self, mode: str, ori_pos: np.ndarray, pos_range: np.ndarray = np.array([0, 0, 0])
        ...
        pos_change = self.obstacle_pos[[1, 0, 2]]  # 绝对位置
        ori_pos_change = ori_pos[[1, 0, 2]]  # 初始位置

        if mode == "fix":
            # gravity bias in z-axis
            # axis have been modified in env init

            self.obstacle_step_position = (pos_change - ori_pos_change) * [-1, -1, 1]
            self._physics.data.qpos[6:9] = self.obstacle_step_position  # 相对于初始位置，要考虑坐标轴的改变
            self._physics.bind(self.obstacle_mocap).mocap_pos[:] = self.obstacle_pos  # 绝对位置
            ...
            self.obstacle_pos = self.obstacle_step_position[[1, 0, 2]]  * [-1, -1, 1] + ori_pos
            self._physics.bind(self.obstacle_mocap).mocap_pos[:] = self.obstacle_pos  # 绝对位置
