...
class Rm63Env_s2_5(gym.Env):
    ...
    def __init__(self, render_mode=None,
        ...
        self.goal_low = np.array(target_original_pos, dtype=np.float32)
        self.goal_high = self.goal_low + np.array(pose_range, dtype=np.float32)
        ...
        self.goal_space = spaces.Box(
            low=self.goal_low, high=self.goal_high, shape=(3,), dtype=np.float32
        )
    ...
    def target_sample(self, mode: str, pos_range: list = [0, 0, 0], ori_pos: list = [0, 0, 0],
                      point_set=None, obs_flag=True):
        """
        ...
        if mode == "range_sample_mode":
            ...
            if point_set is not None:
                ...
                i = random.sample(range(n_point), 1)
                target_xyz = point_set[i, :].reshape([-1])
                self.target_pose = np.array([target_xyz[0], target_xyz[1], target_xyz[2],
                                             1, 0, 0, 0])
                self._target.set_mocap_pose(self._physics,
                                            position=self.target_pose[0:3],
                                            quaternion=self.target_pose[3:]
    ...
    def state2goal(self):
        end_position = self._physics.bind(self._arm.eef_site).xpos
        goal = end_position.reshape([1, -1])
        return goal

    def update_goals(self, goals):
        if torch.is_tensor(goals):
            self.target_set = goals.detach().numpy()
        else:
