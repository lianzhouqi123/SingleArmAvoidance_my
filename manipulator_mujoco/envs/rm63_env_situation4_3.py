...
class Rm63Env_s4_3(gym.Env):
    ...
    def _get_reward(self, dis_t_old, dis_o_old):
        ...
        dis_t = d_t_tran
        if dis_safe < d_danger:  # 进入障碍物的危险距离
            r_avoid = ca * (obs_p1/(dis_safe + obs_p2) - obs_p1/(d_danger + obs_p2))
        else:
            r_avoid = 0
        if dis_t < delta:
        ...
        if dis_t < self.reach_level:  # 到终点
            ...
            terminated = True
        elif contact != 0 or dis_safe < 0:  # 碰障碍物
            reward = rc
            self.flag_cont = True
        else:  # 未到达终点
            reward = r_target + r_avoid

        if self.step_count >= self.step_max - 1 and ~done:  # 循环次数过多
            done = True
            terminated = False
    ...
    def reset(self, seed=None, options=None) -> tuple:
        ...
        # obstacle
        self.obstacle_pos = np.array([-0.5, 0.0, 0.4])
        self.obstacle_control(mode="fix")
